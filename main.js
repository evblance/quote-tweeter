...
$(document).ready(function () {
    ...
    $('#btn-quote').on('click', function () {
     ...
     for (var el = 0; el < data.length; el++) {
       if (data[el].id === quoteID) {
         $(".quote").html(quoteID);
         $(".quote").html("<p class='quote'>\"" + data[el].quote + "\"</p>");
         $(".author").html("<p class='author'>" + " - " + data[el].author + "</p>");
         $('.author').fadeIn(800);
         $('.quote').fadeIn(800);
  ...
  $('#btn-tweet').on('click', function () {
    ...
    var theQuote = $('.quote p').text();
    if ( $('p.quote').text() == "CLICK THE BUTTON TO GENERATE A QUOTE" ) {
      alert("You might want to generate a quote first! :)");
      return;
  
    }
    //var theAuthor = $('.author p').html();
    //var quoteArray = theQuote.split(" ");
    //var authorArray = theAuthor.split(" ");
    
    //var sanitizedQuote = quoteArray.join("%20") + "%0A" + authorArray.join("%20");
    var sanitizedQuote = theQuote + "%0A" + $('.author p').text();
    //https://twitter.com/home?status=
    var tweetStringHref = "https://twitter.com/intent/tweet?text=" + sanitizedQuote;
    
    $('#btn-tweet a').attr('target', "_blank");
    $('#btn-tweet a').attr('href', tweetStringHref);   
